...
public class LeapHand {
    ...
    public float getPinchStrength() { return hand.pinchStrength();}
    /** How much confidence the leap has in its estimation. **/
    public float getConfidence() {
        return hand.confidence();
    }
    public float getPalmRoll(){
    ...
    private LeapHandState createHandState(Hand hand){
        LeapHandState handState = LeapHandState.UNKNOWN;
        if(!hand.isValid()){
            ...
            if(hand.pinchStrength() >= 0.8){
                if(!missingFingers.contains(Finger.Type.TYPE_THUMB)){
                    missingFingers.add(Finger.Type.TYPE_THUMB);
                }
                if(!missingFingers.contains(pinchingFinger.type())){
                    missingFingers.add(pinchingFinger.type());
                }
            }
            ...
            if(missingFingers.size() == 1){
                ...
                handState = LeapHandState.SIGN_NUMBER_THREE;
                    ...
                    areMissingFingers(true, Finger.Type.TYPE_THUMB, Finger.Type.TYPE_RING, Finger.Type.TYPE_PINKY)){
                ...
                handState = LeapHandState.SIGN_NUMBER_ONE;
                    ...
                    || areMissingFingers(true, Finger.Type.TYPE_THUMB, Finger.Type.TYPE_PINKY)){
                handState = LeapHandState.SIGN_NUMBER_THREE;
                ...
                handState = LeapHandState.SIGN_ROCK;
                    ...
                    || areMissingFingers(true, Finger.Type.TYPE_MIDDLE, Finger.Type.TYPE_RING)){
                handState = LeapHandState.SIGN_ROCK;
