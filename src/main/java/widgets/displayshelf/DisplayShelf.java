...
/**
 * Adaption of the official JavaFX sample DisplayShelf. Took the display
 *
...
public class DisplayShelf extends Application {
    ...
    private void init(Stage primaryStage) {
        ...
        for (int i = 0; i < nodes.length; i++) {
            ...
            final Label b = new Label("Node "+i);
            nodes[i] = b;
            //b.setOnAction(event -> b.setText("Clicked"));
//            ((StackPane) nodes[i]).getChildren().addAll(b);
        }
        // create display shelf
        DisplayShel displayShelf = new DisplayShel(nodes);
        displayShelf.setPrefSize(WIDTH, HEIGHT);
        root.getChildren().add(displayShelf);

        displayShelf.setPickOnBounds(false);
    }

    /**
     * A ui control which displays a browsable display shelf of images
     */
    public static class DisplayShel extends Region {
        ...
        public DisplayShel(Node[] nodes) {
            ...
            // create items
            items = new PerspectivePane[nodes.length];
            for (int i=0; i<nodes.length; i++) {
                final PerspectivePane node =
                        items[i] = new PerspectivePane(nodes[i]);
            ...
            this.addEventHandler(MouseEvent.MOUSE_CLICKED, new EventHandler<MouseEvent>() {
                ...
                public void handle(MouseEvent event) {
                    ...
                    boolean isConsumed = event.isConsumed();
            ...
            this.addEventFilter(MouseEvent.MOUSE_CLICKED, new EventHandler<MouseEvent>() {
                ...
                public void handle(MouseEvent event) {
                    ...
                    boolean isConsumed = event.isConsumed();
                    System.out.println("Clicked at: "+event.getX()+" "+event.getY());
                    System.out.println("Item at: "+items[2].getTranslateX()+" "+items[2].getTranslateY());
                    System.out.println("Item layout at: "+ items[2].getLayoutX()+" "+items[2].getLayoutY());
        ...
        @Override protected void layoutChildren() {
            ...
            centered.setLayoutY((getHeight() - PerspectivePane.HEIGHT) / 2);
            centered.setLayoutX((getWidth() - PerspectivePane.WIDTH) / 2);
        ...
        private void update() {
            ...
            for (int i = 0; i < left.getChildren().size(); i++) {
                final PerspectivePane it = items[i];
            ...
            final PerspectivePane centerItem = items[centerIndex];
            keyFrames.add(new KeyFrame(DURATION,
            ...
            for (int i = 0; i < right.getChildren().size(); i++) {
                final PerspectivePane it = items[items.length - i - 1];
        ...
        private void shiftToCenter(PerspectivePane item) {
            for (int i = 0; i < left.getChildren().size(); i++) {
     ...
     */
    public static class PerspectivePane extends Parent {
        ...
        public PerspectivePane(Node node) {
            AnchorPane pane = new AnchorPane(node);
            pane.setEffect(ReflectionBuilder.create().fraction(REFLECTION_SIZE).build());
            setEffect(transform);
